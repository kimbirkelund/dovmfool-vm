...
namespace gppg
{
        ...
        where YYLTYPE : IMerge<YYLTYPE>
    {
        ...
        private class YYException : Exception { }
        private class AcceptException : YYException { }
        private class AbortException : YYException { }
        private class ErrorException : YYException { }
        protected void YYAccept() { throw new AcceptException(); } 
        protected void YYAbort() { throw new AbortException(); }
        protected void YYError() { throw new ErrorException(); }
        ...
        protected void Reduce(int rule_nr)
        {
            ...
            if (rule.rhs.Length == 1)
            {
                ...
                yyloc = location_stack.Top(); // Default action "@$ = @1;
            }
            else
            {
                if (rule.rhs.Length == 0)
                {
                    ...
                    // Explicit semantic action may mutate the value
                    yyval = default(YYSTYPE);
