...
namespace GPLEX.Automaton
{
    internal class PartitionBlock
    {
        ...
        private int partitionOrd;
        private int symbolsLeft;                   //  Number of symbols left on the "pair-list" for this block.
        private int generation = 0;                //  The current splitting generation.
        private int predCount = 0;                 //  Number of predecessors from the current generation.
        internal PartitionBlock twinBlk = null;    //  During a split, the two fragments reference each other.
        internal LinkedList<DFSA.DState> members;

        public int Ord { get { return partitionOrd; } }
        ...
        internal void Diagnose(string s)
        {
            Console.WriteLine("PartitionBlock {0} : {1}", Ord, s);
            foreach (DFSA.DState dSt in this.members) dSt.Diagnose();
        }

        internal void PairDiag(int sym)
        {
            Diagnose(String.Format("symbol \"{0}\"", Utils.Map((char)sym)));
        }
    }
    ...
    public class Minimizer
    {
        ...
        internal void PopulatePartitions(List<DFSA.DState> list)
        {
            ...
            if (dfsa.myTask.Trace)
            {
                Console.WriteLine("{0} Initial blocks", allBlocks.Count);
                foreach (PartitionBlock b in allBlocks) b.Diagnose("initial");
            }
        }
                ...
                if (DFSA.SpansEqual(first.accept.aSpan, dSt.accept.aSpan))
                    ...
                    else
                    {
                        ...
                        if (first.rhCntx > 0 && first.rhCntx == dSt.rhCntx)
                            // From now on only add states with mathing rhs length
        ...
        internal DFSA.DState PMap(DFSA.DState dSt)
        {
        ...
        internal void RefinePartitions()
        {
            ...
            while (worklist.Count > 0)
            {
                ...
                if (dfsa.myTask.Trace && doneSplit)
                {
                    doneSplit = false;
                    Console.WriteLine("{0} blocks now", allBlocks.Count);
                    foreach (PartitionBlock b in allBlocks) b.Diagnose(generation.ToString());
                }
                        ...
                        if (lBlk.PredCount != lBlk.MemberCount) // Need to split this block
                        {
                            ...
                            if (tBlk == null)
                            {
                                ...
                                if (dfsa.myTask.Trace)
                                {
                                    Console.WriteLine();
                                    Console.WriteLine(
                                        "Splitting block {0} on (block,symbol) pair ({1},'{3}'), creating new block {2}",
                                        lBlk.Ord, blk.Ord, tBlk.Ord, Utils.Map((char)sym));
                                    doneSplit = true;
                                }
                            }
            ...
            if (dfsa.myTask.Trace)
            {
                Console.WriteLine("{0} Final blocks", allBlocks.Count);
                foreach (PartitionBlock b in allBlocks) b.Diagnose("final");
            }
        }
    }
}